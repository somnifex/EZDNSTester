...
async def run_test(test_req: TestRequest):
    ...
    if test_req.type == "udp":
        return dns_tester.test_udp(server, test_req.domain)
    elif test_req.type == "dot":
        return dns_tester.test_dot(server, test_req.domain)
    elif test_req.type == "doh":
        return await dns_tester.test_doh(server, test_req.domain, test_req.proxy)
    else:
